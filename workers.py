...
#     for page_number, page_content in doc.items():
#         q = qGen.generate_questions_dict(page_content)
#         for i in range(1, len(q) + 1):
#             temp = []
#             for j in range(len(q[i]['options'])):
#                 temp.append(q[i]['options'][j])
#             q[i]['options'] = temp
#             q_with_options[f"Page {page_number}, Question {i}"] = q[i]

#     return q_with_options
